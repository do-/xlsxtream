...
module.exports = class {
	...
	async load () {
		...
		for (const r of this.relationships) 
			...
			if (r.Type === TYPE_WORKSHEET) 
					
				this.sheets.push (this.sheetByName [this.sheetId2Name.get (r.Id)] = new Worksheet (this, r.Id))								
	...
	async parseXML (name, raw = false) {
		...
		const root = parser.process (xml.toString ())
	...
	async loadWorkbook () {
		...
		this.sheetId2Name = new Map (); for (const {localName, children} of (await this.parseXML ('xl/workbook.xml', true)).children) if (localName === 'sheets') {
